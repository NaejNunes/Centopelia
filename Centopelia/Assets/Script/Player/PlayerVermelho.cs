...
public class PlayerVermelho : MonoBehaviour
{
	public 		GameObject 		PAmarelo, PAzul, PMarrom, PRosa, PRoxo, PVerde, PAumenta, PDiminue;
	private 	int			    Item;
	...
	void OnTriggerEnter(Collider BallMudaCor)
	{
		...
		else if (BallMudaCor.gameObject.tag == "BVermelha")
		{		
			Pontuacao.Pontos += 5;
			Destroy(gameObject);
